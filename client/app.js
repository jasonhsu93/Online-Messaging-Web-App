...
function main() {
    ...
    cpen322.export(arguments.callee, {chatView, lobby});
}
    ...
    addMessage(message, prepend = false) {
        ...
        const newMessage = createDOM(`
            <div class="message${message.username === profile.username ? ' my-message' : ''}">
                ...
                <button class="summary-button" data-message="${sanitizedText}">Summarize</button>
            </div>
        `);

        newMessage.querySelector('.summary-button').addEventListener('click', (event) => {
