    ...
    db.getRoom(roomId)
        .then(room => {
            ...
            return axios.post(
                ...
                {
                    model: "llama3.2",
                    prompt: `Summarize the following chat messages in a few sentences: ${messages}`,
                    stream: false
                },
...
app.post('/chat/:room_id/message/summary', sessionManager.middleware, (req, res) => {
    ...
    axios.post(
        ...
        {
            model: "llama3.2",
            prompt: `Summarize the following message in a few sentences: ${message}`,
            stream: false
        },
        { headers: { 'Content-Type': 'application/json' } }
    )
...
cpen322.export(__filename, { app, db, messages, messageBlockSize, isCorrectPassword, sessionManager});
